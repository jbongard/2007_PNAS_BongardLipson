...
public:
	...
	EXP_PHASE(MODEL **m, MATRIX *targetData);
	~EXP_PHASE(void);
	TEST *Evolve(int numT1, TEST **t1, int numT2, TEST **t2, MATRIX *targetData);

private:
	void ComputeFinalValues(RESULT **results, MATRIX *finalValues);
	void CreateChildPopulation(MATRIX *targetData);
	void CreateHorizontalResults(RESULT **results, TEST **testsToEvaluate, int i);
	void CreateNewClimbers(MATRIX *targetData);
