...
void EE::PerformInference(void) {
	...
	tests[numTests++] = new TEST;
	//tests[numTests-1]->ZeroIt();
...
void EE::Explore(int currIteration) {
	...
	if ( simParams->evolvingTests ) {
		...
		explorationPhase = new EXP_PHASE(models);

		tests[numTests++] = explorationPhase->Evolve(numTests,tests,numBankedTests,bankedTests);
...
void EE::LoadTarget(void) {
	...
	(*inFile) >> simParams->numParams;
	(*inFile) >> simParams->numEqs;
...
void EE::SaveRealFitness(int append) {
	...
	for (int i=0;i<EVALS_FOR_OBJ_FITNESS;i++) {
		...
		simParams->evaluatingTarget = true;
		target->Evaluate(t,targetResult,STARTING_EXPERIMENT_LENGTH);
		simParams->evaluatingTarget = false;

		models[0]->Evaluate(t,modelResult,STARTING_EXPERIMENT_LENGTH);
